#!/usr/bin/env python3
# クラスター②(PLM・BOM) / ③(類似検索・AI) の記事(各10本)を確定原稿から .md 生成。
# 本文・数値・出典は逐語コピー（言い換え・捏造しない）。型は gen_guide_articles.py と同じ。
import json, re, os, sys

HOME = os.path.expanduser('~')
SRC2 = os.path.join(HOME, 'Downloads', 'files', 'ARCHAIVE_クラスター②_PLM-BOM.md')
SRC3 = os.path.join(HOME, 'Downloads', 'files', 'ARCHAIVE_クラスター③_類似検索-AI.md')
...
T2S = {
    # ② PLM・BOM
    "中小製造業のPLM代替とは": ("plm-alternative-sme", "pillar", "plm"),
    "PLMとは": ("plm", "spoke", "plm"),
    "PDMとは": ("pdm", "spoke", "plm"),
    "ERPとは": ("erp", "spoke", "plm"),
    "生産管理システムとは": ("production-management-system", "spoke", "plm"),
    "製品ライフサイクル管理とは": ("product-lifecycle-management", "spoke", "plm"),
    "設計BOMと製造BOMの違い": ("ebom-mbom", "spoke", "plm"),
    "設計変更管理とは": ("design-change-management", "spoke", "plm"),
    "構成管理とは": ("configuration-management", "spoke", "plm"),
    "トレーサビリティとは": ("traceability", "spoke", "plm"),
    # ③ 類似検索・AI
    "製造業の暗黙知を形式知化するAIとは": ("tacit-knowledge-ai", "pillar", "ai-search"),
    "形状検索・3D類似検索とは": ("shape-search", "spoke", "ai-search"),
    "特徴量・ベクトル検索とは": ("vector-search", "spoke", "ai-search"),
    "セマンティック検索とは": ("semantic-search", "spoke", "ai-search"),
    "自然言語検索・チャット型検索とは": ("nl-search", "spoke", "ai-search"),
    "RAG（検索拡張生成）とは": ("rag", "spoke", "ai-search"),
    "生成AIと製造業とは": ("generative-ai-manufacturing", "spoke", "ai-search"),
    "AIエージェントとは": ("ai-agent", "spoke", "ai-search"),
    "AI見積・自動見積とは": ("ai-estimation", "spoke", "ai-search"),
    "ナレッジマネジメントとは": ("knowledge-management", "spoke", "ai-search"),
}

# 関連用語/中核記事の表記 → slug（新規＋図面管理クラスター既出）。未登録は WARN（製造業AIデータハブ等は未執筆）。
...
# 診断ツールを付けるピラー
DIAGNOSIS = {"plm-alternative-sme": "plm-alternative"}
EYEBROW = {"plm-alternative-sme": "判断ガイド", "tacit-knowledge-ai": "AIと製造業"}

TRAILING = ['を', 'は', 'が', 'の', 'に', 'へ', 'で', 'と', 'も', 'や', 'ね', 'よ', 'か', 'な', 'さ']
...
def build_md(a, order):
    ...
    if typ == 'pillar':
        ...
        if slug in DIAGNOSIS:
            fm.append(f'diagnosis: {q(DIAGNOSIS[slug])}')
        body = a['body']
    else:
        body = (a['intro'] + '\n\n' + a['body']).strip()
