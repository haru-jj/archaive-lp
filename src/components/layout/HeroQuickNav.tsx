...
export default function HeroQuickNav() {
  return (
    <section className="bg-white h-full w-full border-t border-gray-200 flex">
      <div className="container mx-auto px-0 sm:px-4 h-full flex-1 flex">
        <div className="flex overflow-x-auto snap-x divide-x divide-gray-200 sm:grid sm:grid-cols-2 lg:grid-cols-6 sm:overflow-visible sm:divide-y-0 sm:divide-x h-full w-full">
...
function QuickNavItem({ href, title, label }: { href: string; title: string; label: string }) {
  ...
  return (
    <a
      href={href}
      onClick={handleClick}
      className="group flex h-full w-[180px] shrink-0 snap-start sm:w-full sm:shrink items-center justify-between min-h-[72px] sm:min-h-[80px] px-4 py-4 hover:bg-gray-50 transition-all duration-300 relative overflow-hidden"
    >
