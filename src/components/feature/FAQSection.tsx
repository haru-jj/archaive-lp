...
const faqs: FAQItem[] = [
  {
    question: '無料デモやトライアルで、実際の使い勝手を確認できますか？',
    answer:
      'はい、可能です。弊社が用意しておりますデータによる無料デモに加え、貴社の実際の図面データを用いたトライアル利用（有償PoC）も実施しております。「自社の図面がどう整理されるか」をお試しいただけます。',
  },
  {
    question: '過去データの移行サポート（導入時や導入後のサポート）はありますか？',
    answer:
      '専任スタッフによるデータ移行サポートが可能です。既存のファイルサーバーからのデータ移行支援を行っておりますので、大量の図面がある場合でも最適な移行プランをご提案します。',
  },
  {
    question: '図面は機密情報ですが、セキュリティ対策は万全ですか？',
    answer:
      '世界最高水準のセキュリティ基準を持つAWSを採用し、通信の暗号化や詳細なアクセス権限設定機能も備えています。官公庁や大手企業でも採用される管理体制で技術情報を安全に保護します。',
  },
  {
    question: '専用ソフトのインストールは必要ですか？スマホやタブレットでも見られますか？',
    answer:
      'インストールは不要で、ブラウザさえあれば利用可能です。パソコンはもちろん、タブレットやスマートフォンからも図面の閲覧・検索ができ、工場内や外出先など場所を選ばず最新の図面情報にアクセスできます。',
  },
  {
    question: 'ファイル名が整理されていなくても、AIで図面を探せますか？',
    answer:
      'はい、ファイル名に依存せず「中身」で検索可能です。AI-OCRが図面内の「図番」「品名」「材質」「顧客名」などを自動でテキスト化するため、記載情報さえ分かれば瞬時に検索・発見できます。',
  },
];
